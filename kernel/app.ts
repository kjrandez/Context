...
class PageEntry {
	...
	constructor(public element: Entity) {
		this.key = PageEntry.nextKey++;
	}

	model() {
		return {key: this.key, element: this.element};
	}
}
...
class Page extends Entity
{
	...
	constructor(content: Entity[]) {
		super()
		this.entries = content.map((x) => this.resolvedEntry(x))
	}

	value() {
		return {
			entries: this.entries.map((x) => x.model()),
			latestEntry: null
		}
	}

	resolvedEntry(item: Entity | PageEntry): PageEntry {
		if (item instanceof Entity)
			return new PageEntry(item);
		else
			return item;
	}
}
