...
def default(observer):
    return Page(observer, [
        Text(observer, '### Hello world'),
        Text(observer, 'How are you doing today?'),
        Text(observer, "I'm doing just fine thank you very much."),
        Page(observer, [
            Page(observer, [
                Text(observer, 'Introduction'),
                Text(observer, lorem1)
            ]),
            Page(observer, [
                Text(observer, 'Mas informacion'),
                Text(observer, lorem2)
            ]),
            Page(observer, [
                Text(observer, '[Scope_0]'),
                Image(observer, 'lol2.png', "Funny joke")
            ])
        ], column=True),
        Page(observer, [
            Text(observer, lorem3)
        ], column=True),
        Image(observer, 'sw.png', "Switching characteristics"),
        Page(observer, [
            Text(observer, "This is pretty funy if you actually think about it for a second."),
            Text(observer, "I really can't agree with that statement")
        ]),
        Text(observer, 'Just to finish it off right here.'),
        Text(observer, "Actually I'll go ahead and add a bit more to be sure...")
    ])
